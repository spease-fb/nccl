...
#define PROF_TYPE_IBV_OPEN_DEVICE 4
#define PROF_TYPE_IBV_CLOSE_DEVICE 5
#define PROF_TYPE_IBV_QUERY_PORT 6
#define PROF_TYPE_IBV_QUERY_GID 7
#define PROF_TYPE_IBV_DESTROY_QP 8
#define PROF_TYPE_IBV_CREATE_QP 8
#define PROF_TYPE_IBV_MODIFY_QP 9
...
ncclResult_t wrap_ibv_open_device(struct ibv_context **ret, struct ibv_device *device) { /*returns 0 on success, -1 on failure*/
  ...
  int profId;
  NCCLCHECK(ncclProfEventStart(g_currentProfilerRecord, &profId, PROF_TYPE_IBV_OPEN_DEVICE, 1, device));
  return ncclret;
}
...
ncclResult_t wrap_ibv_close_device(struct ibv_context *context) { /*returns 0 on success, -1 on failure*/
  ncclResult_t ncclret = ncclSuccess;
  int profId;
  NCCLCHECK(ncclProfEventStart(g_currentProfilerRecord, &profId, PROF_TYPE_IBV_CLOSE_DEVICE, 1, context));
...
ncclResult_t wrap_ibv_query_port(struct ibv_context *context, uint8_t port_num, struct ibv_port_attr *port_attr) { /*returns 0 on success, or the value of errno on failure (which indicates the failure reason)*/
  ...
  int profId;
  NCCLCHECK(ncclProfEventStart(g_currentProfilerRecord, &profId, PROF_TYPE_IBV_QUERY_PORT, 3, context, port_num, port_attr));
  return ncclret;
}
...
ncclResult_t wrap_ibv_query_gid(struct ibv_context *context, uint8_t port_num, int index, union ibv_gid *gid) {
  ...
  int profId;
  NCCLCHECK(ncclProfEventStart(g_currentProfilerRecord, &profId, PROF_TYPE_IBV_QUERY_GID, 5, context, port_num, index, gid->global.subnet_prefix, gid->global.interface_id));
   return ncclret;
}
...
ncclResult_t wrap_ibv_destroy_qp(struct ibv_qp *qp) {
  ncclResult_t ncclret = ncclSuccess;
  int profId;
  NCCLCHECK(ncclProfEventStart(g_currentProfilerRecord, &profId, PROF_TYPE_IBV_DESTROY_QP, 1, qp));
...
ncclResult_t wrap_ibv_create_qp(struct ibv_qp **ret, struct ibv_pd *pd, struct ibv_qp_init_attr *qp_init_attr) {
  ...
  int profId;
  NCCLCHECK(ncclProfEventStart(g_currentProfilerRecord, &profId, PROF_TYPE_IBV_CREATE_QP, 2, *ret, qp_init_attr));
  return ncclret;
}
...
ncclResult_t wrap_ibv_modify_qp(struct ibv_qp *qp, struct ibv_qp_attr *attr, int attr_mask) { /*returns 0 on success, or the value of errno on failure (which indicates the failure reason)*/
  ncclResult_t ncclret = ncclSuccess;
  int profId;
  NCCLCHECK(ncclProfEventStart(g_currentProfilerRecord, &profId, PROF_TYPE_IBV_MODIFY_QP, 2, qp, attr, attr_mask));
